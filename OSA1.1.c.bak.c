...
Thread scheduler(Thread origThread){
    ...
    if(t == origThread) return mainThread;
    return t;
}
...
void associateStack(int signum) {
    ...
    if (setjmp(localThread->environment) != 0) { // will be zero if called directly
        ...
        switcher(localThread,scheduler(localThread)); // TODO: at the moment back to the main thread, should remove the current thread from the schedule and allocate next one
    }
}
...
int main(void) {
    ...
    switcher(mainThread,scheduler(mainThread));
